...
func showHelp() {
	...
	fmt.Println()
	fmt.Println("  CI / Evaluation:")
	fmt.Println("    cyntr eval <path>            Run eval cases (file or dir), exit non-zero on fail")
	fmt.Println("    cyntr eval ./evals/ --format junit --output results.xml")
	fmt.Println()
