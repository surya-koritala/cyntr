...
func (o *OpenAI) Name() string { return "gpt" }
...
func (o *OpenAI) Chat(ctx context.Context, messages []agent.Message, tools []agent.ToolDef) (agent.Message, error) {
	...
	if err != nil {
		return agent.Message{}, fmt.Errorf("OpenAI API: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		b, _ := io.ReadAll(resp.Body)
		return agent.Message{}, fmt.Errorf("OpenAI error %d: %s", resp.StatusCode, string(b))
	}
...
func (o *OpenAI) parseResponse(resp openaiResponse) agent.Message {
